...
from apps.chat.prompts import CHAT_SYSTEM_PROMPT, ROUTER_SYSTEM_PROMPT
...
def _call_queries(questions_data: list, user_id: UUID, chat_id: UUID):
    ...
    for item in questions_data:
        ...
        all_docs.append(
            {"question": item.get("question"), "docs": [doc["text"] for doc in top_3]}
        )
...
async def start_query_processing(user_id: UUID, content: str, chat_id: UUID):
    ...
    async with WorkerSessionLocal() as session:
        await create_message(
            session, content=full_answer, role="system", chat_id=chat_id
        )
...
def start_document_processing(
    ...
    parallel_tasks = [
        index_document_chunks.s(
            user_id,
            chat_id,
            document_id,
            chunks[i : i + BATCH_SIZE],
            list(range(i, i + len(chunks[i : i + BATCH_SIZE]))),
        )
