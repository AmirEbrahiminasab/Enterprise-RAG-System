...
from config.database import Chat, Document, Message, User, create_database, get_session
...
from .documents.services import create_chat, create_document, create_message
...
async def get_messages(
    ...
    query = (
        select(Message)
        .join(Chat)
        .where(Message.chat_id == chat_id, Chat.user_id == current_user.id)
    )
    result = await session.execute(query)
...
async def fetch_documents(
    ...
    query = (
        select(Document)
        .join(Chat)
        .where(Document.chat_id == chat_id, Chat.user_id == current_user.id)
    )
    result = await session.execute(query)
